"""
HF-201 — Tests for the Text Generation lesson.
...
SCRIPT = LESSON_DIR / "01_Text_Generation.py"
...
def test_script_compiles() -> None:
    ...
    ast.parse(source)  # raises SyntaxError on invalid Python
    compile(source, str(SCRIPT), "exec")


def test_documentation_notebook_exists() -> None:
    nb = LESSON_DIR / "documentation" / "01_Text_Generation.ipynb"
...
def test_documentation_notebook_is_valid() -> None:
    ...
    nb_path = LESSON_DIR / "documentation" / "01_Text_Generation.ipynb"
...
def test_text_generation_pipeline_available() -> None:
    transformers = pytest.importorskip("transformers", reason="transformers not installed")
    assert "text-generation" in transformers.pipelines.SUPPORTED_TASKS
